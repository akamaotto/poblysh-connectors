...
export default function SignalDetailPage() {
  ...
  const connection =
    connections.find((c) => signal && c.id === signal.connectionId) ?? null;
  const groundedSignal = groundedSignals.find(
    (gs) => gs.sourceSignalId === signalId,
  );
  ...
  const handleGround = async () => {
    // Using the current signal from context for grounding
    ...
    try {
      ...
      const newGroundedSignal = generateGroundedSignal(
        signal,
        signals,
        connections,
      );
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <p className="text-sm text-gray-600 mb-4">
                Signal grounding is the process of analyzing a signal across all
                connected providers to find supporting evidence, calculate
                confidence scores, and provide dimensional analysis of the
                signal&apos;s importance and reliability.
              </p>
              ...
              <p className="text-sm text-gray-600">
                The grounding engine searches for related activities, mentions,
                references, and cross-provider correlations that strengthen or
                weaken the original signal.
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mb-4">
                Each grounded signal receives an overall confidence score based
                on multiple dimensions: relevance, impact, timeliness,
                authority, and corroboration from evidence.
              </p>
              ...
              <p className="text-sm text-gray-600">
                The more providers you connect, the stronger the grounding
                results become. Cross-provider evidence provides the highest
                confidence scores and most reliable signal analysis.
              </p>
            </div>
          </div>
        </div>
        ...
        {groundedSignal && (
          <div className="mt-8 bg-green-50 border border-green-200 rounded-lg p-6">
            ...
            <p className="text-green-800 mb-6">
              This signal has been analyzed with{" "}
              {groundedSignal.evidence.length} pieces of supporting evidence and
              received a confidence score of {groundedSignal.score}%.
            </p>
