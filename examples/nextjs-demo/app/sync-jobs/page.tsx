...
export default function SyncJobsPage() {
  ...
  const generateFailedSyncJobs = (connection: DemoConnection): DemoSyncJob[] => {
  ...
  useEffect(() => {
    if (state.connections.length > 0) {
      ...
      const failedJobs = state.connections.slice(0, 2).flatMap(connection =>
  ...
  const handleRetryJob = (jobId: string) => {
    ...
    setSyncJobs(prevJobs =>
      prevJobs.map(job =>
        job.id === jobId
          ? { ...job, status: 'running' as const, errorMessage: undefined, errorCount: 0 }
          : job
      )
    );
  };
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600">
                Monitor and manage sync operations across your connected services
              </p>
            </div>

            <nav className="flex items-center space-x-4" aria-label="Page actions">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" role="main">
        {state.connections.length === 0 ? (
          <section className="bg-white border border-gray-200 rounded-lg p-8 text-center" aria-labelledby="no-connections-title">
            ...
            <h2 id="no-connections-title" className="text-lg font-medium text-gray-900 mb-2">
