...
- reqwest (dev-deps) = 0.12.9

Selected additions (runtime deps):
- reqwest = 0.12.9 (features: `json`, `rustls-tls`) — HTTP client for Gmail REST and JWKS
- oauth2 = 5.0.0 — Authorization code + refresh token flows for Gmail
...
- backoff = 0.4.0 — Exponential backoff for Gmail API retries
...
- OIDC verification uses Google public keys (JWKS) and standard RS256 verification; we validate `iss`, `aud`, and `email`/`sub` claims as recommended.
...
## Flow
1) Pub/Sub pushes HTTPS POST to our public webhook `POST /webhooks/gmail` with optional OIDC token in `Authorization: Bearer <JWT>`.
2) Handler verifies OIDC JWT:
   - Fetch JWKS from Google and cache (validate `kid`, signature, `iss` in {`https://accounts.google.com`, `accounts.google.com`}, `aud` equals configured audience, and token not expired).
   - Optionally verify `email`/`sub` matches the configured service account on the subscription.
...
4) Enqueue a sync job using `message.messageId` as the preferred idempotency key (record `subscription` for diagnostics). Fall back to `(connection_id, historyId)` only if `messageId` is unavailable. Respond `202 Accepted` immediately to ack (push acks on any 2xx).
...
## Security
- Public webhook path allowed when OIDC verification succeeds; otherwise reject with 401.
...
- Gmail HTTP retries with `backoff` on 429/5xx with jitter, honoring `Retry-After`.
- Duplicate deliveries are ignored by idempotent job insert (`UNIQUE(connection_id, historyId)` or `dedupe_key`).
- `historyId` invalid (e.g., too old): record condition and schedule re‑sync (bounded backfill) with a distinct job type; emit an internal signal for observability.
