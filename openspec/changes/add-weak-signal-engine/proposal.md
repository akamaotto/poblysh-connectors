...
## What Changes
- Introduce a background service/module (`weak_signal_engine`) that:
...
- Code: new module (likely `src/signals/weak_engine.rs`), DB migrations for grounded signals table, background worker wiring.
- APIs: optional new `GET /grounded-signals` endpoint (or extend `/signals` with filters); notifications to PR teams.
