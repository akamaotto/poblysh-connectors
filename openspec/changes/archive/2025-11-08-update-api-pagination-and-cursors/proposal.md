...
- Define a core, cross-cutting pagination contract in `api-core`:
  ...
  - Response shape: `next_cursor` always present as a string field; set to `null` when there are no additional pages. This provides a consistent contract for clients - they can always check for the existence and nullability of the field.
  ...
  - Cursor token: opaque, standard Base64-encoded JSON of last item's sort keys (aligned with existing `src/cursor.rs` implementation).
...
- Adding encryption or signatures to cursors (tokens are opaque with comprehensive server-side validation for security).
