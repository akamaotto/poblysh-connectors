connectors/openspec/changes/add-connectors-integration-guide/specs/integration-guide/spec.md
# Spec: Connectors Integration Guide

## Overview

This spec defines a concise, implementation-aligned integration guide for the Poblysh Connectors Service.

The guide’s purpose is to:

- Explain the role of the Connectors Service in the Poblysh ecosystem.
- Define how Poblysh Core and frontends integrate with it in a consistent, safe way.
- Clarify how tenancy and scoping work (especially `X-Tenant-Id`).
- Describe end-to-end flows for:
  - Listing providers
  - Initiating OAuth connector flows
  - Handling OAuth callbacks
  - Listing connections
  - Fetching signals
- Provide a curated endpoint reference targeted at Poblysh product engineers.
- Rely on the existing OpenAPI and not conflict with it.

This spec governs the content and scope of the `connectors-service` integration guide document (exact path defined below) and how it should be maintained.
...
If future changes adjust Connectors endpoints or authentication, corresponding updates MUST be proposed as separate OpenSpec changes that update this integration guide spec accordingly.
...
- The integration guide produced from this spec MUST:
  - Live in a clear, discoverable location (e.g. `docs/integration/connectors-service.md`).
  - Be kept relatively short and focused; it MUST NOT duplicate the entire OpenAPI.
  - Use concrete, realistic examples grounded in current behavior.
- Cross-reference:
  - `add-tenant-mapping-and-signals-ux` for deeper rules on tenant mapping and signals UX.
  - Connector-specific changes (e.g. GitHub, Jira) for provider-specific details.
