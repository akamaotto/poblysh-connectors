...
## Decisions
- Cursor token format: Base64URL-encoded JSON object
  - Shape: `{ "ver": 1, "keys": { ...ordered sort keys... } }`
  - Example (event-like): `{ "ver":1, "keys":{ "created_at":"2024-11-01T12:00:00Z", "id":"11111111-1111-1111-1111-111111111111" } }`
  - Encoding: Base64 URL-safe, no padding
  - Rationale: opaque to clients, forwards-compatible via `ver`, easy to debug server-side
...
## Risks / Trade-offs
- Cursor visibility: Base64 without signing means clients could tamper; mitigation is server-side validation + treating tokens as hints. Signing could be added later (`ver=2`).
...
2) Add a small internal `cursor` module to encode/decode with Base64URL JSON.
...
- Base64 `0.22` (to add): URL-safe encoding for opaque tokens
  - Docs: https://docs.rs/base64/0.22.1/base64/

## Open Questions
- Should we sign cursors to prevent tampering? (Out of scope for v1; consider HMAC in ver=2.)
