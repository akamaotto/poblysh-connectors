...
impl AuthPlugin {
    pub(super) async fn scramble(
        ...
        match self {
            ...
            AuthPlugin::Sha256Password => encrypt_rsa(stream, password).await,
        ...
        match self {
            AuthPlugin::CachingSha2Password if packet[0] == 0x01 => {
                match packet[1] {
                    ...
                    // AUTH_CONTINUE
                    0x04 => {
                        let payload = encrypt_rsa(stream, password).await?;
...
async fn encrypt_rsa(stream: &mut MySqlStream, password: &str) -> Result<Vec<u8>, Error> {
    if stream.is_tls {
    ...
    Err(err_protocol!(
        "sha256_password authentication over insecure connections requires RSA support, \
         which is disabled in this build. Enable TLS to use this authentication plugin."
    ))
}
